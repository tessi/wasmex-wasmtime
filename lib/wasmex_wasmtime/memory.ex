defmodule WasmexWasmtime.Memory do
  @moduledoc """
  A WebAssembly instance has its own memory, represented by the `WasmexWasmtime.Memory` struct.
  It is accessible by the `WasmexWasmtime.Instance.memory/3` getter.

  The `grow/2` method allows to grow the memory by a number of pages (of 64 kB or 65,536 bytes each).

  ```elixir
  WasmexWasmtime.Memory.grow(memory, 1)
  ```

  The current size of the memory can be obtained with the `length/1` method:

  ```elixir
  WasmexWasmtime.Memory.length(memory) # in bytes, always a multiple of the the page size (64 kB)
  ```
  ...
  The `WasmexWasmtime.Memory` struct views the WebAssembly memory of an instance as an array of values of different types.
  ...
  ```elixir
  {:ok, memory} = WasmexWasmtime.memory(instance, :uint16, 0)
  WasmexWasmtime.Memory.bytes_per_element(memory) # 2
  ```

  Since the same memory seen in different data types uses the same buffer internally. Let's have some fun:

  ```elixir
  int8 = WasmexWasmtime.memory(instance, :int8, 0)
  int16 = WasmexWasmtime.memory(instance, :int16, 0)
  int32 = WasmexWasmtime.memory(instance, :int32, 0)
  ...
  @spec from_instance(WasmexWasmtime.Instance.t()) :: {:ok, t} | {:error, binary()}
  def from_instance(%WasmexWasmtime.Instance{} = instance) do
    from_instance(instance, :uint8, 0)
  end

  @spec from_instance(WasmexWasmtime.Instance.t(), atom(), non_neg_integer()) ::
          {:ok, t} | {:error, binary()}
  def from_instance(%WasmexWasmtime.Instance{resource: resource}, size, offset)
      when size in [:uint8, :int8, :uint16, :int16, :uint32, :int32] do
    case WasmexWasmtime.Native.memory_from_instance(resource) do
  ...
  ```elixir
  {:ok, memory} = WasmexWasmtime.Instance.memory(instance, :uint16, 0)
  WasmexWasmtime.Memory.bytes_per_element(memory) # 2
  ```
  ...
  ```elixir
  WasmexWasmtime.Memory.bytes_per_element(:uint32) # 4
  ```
  """
  ...
  def bytes_per_element(size) do
    WasmexWasmtime.Native.memory_bytes_per_element(size)
  end
  ...
  ```elixir
  {:ok, memory} = WasmexWasmtime.Memory.from_instance(instance, :uint8, 0)
  WasmexWasmtime.Memory.length(memory) # 1114112 (17 * 65_536)
  ```
  """
  ...
  ```elixir
  {:ok, memory} = WasmexWasmtime.Instance.memory(instance)
  WasmexWasmtime.Memory.length(memory, :uint8, 0) # 1114112 (17 * 65_536)
  ```
  """
  @spec length(t, atom(), non_neg_integer()) :: pos_integer()
  def length(%__MODULE__{resource: resource}, size, offset) do
    WasmexWasmtime.Native.memory_length(resource, size, offset)
  end
  ...
  def grow(%__MODULE__{resource: resource}, pages) do
    WasmexWasmtime.Native.memory_grow(resource, pages)
  end
  ...
  def get(%__MODULE__{resource: resource}, size, offset, index) do
    WasmexWasmtime.Native.memory_get(resource, size, offset, index)
  end
  ...
  def set(%__MODULE__{resource: resource}, size, offset, index, value) do
    WasmexWasmtime.Native.memory_set(resource, size, offset, index, value)
  end
  ...
  def write_binary(%__MODULE__{resource: resource}, size, offset, index, str)
    ...
    WasmexWasmtime.Native.memory_write_binary(resource, size, offset, index, str)
  end

  @spec read_binary(t, non_neg_integer(), non_neg_integer()) :: binary()
  def read_binary(%WasmexWasmtime.Memory{} = memory, index, length) do
  ...
  def read_binary(%__MODULE__{resource: resource}, size, offset, index, length) do
    WasmexWasmtime.Native.memory_read_binary(resource, size, offset, index, length)
  end
...
defimpl Inspect, for: WasmexWasmtime.Memory do
  import Inspect.Algebra

  def inspect(dict, opts) do
    concat(["#WasmexWasmtime.Memory<", to_doc(dict.reference, opts), ">"])
  end
end
